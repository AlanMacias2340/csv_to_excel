...
Image conversion: POST `/api/v1/convert-image` accepts PNG uploads and returns a single `.webp` (for one image) or a ZIP archive of `.webp` files (for multiple images). POST `/api/v1/convert-webp` accepts WebP uploads and returns PNG files. POST `/api/v1/convert-pdf` accepts PDF files and converts each page to PNG (returns single PNG for 1-page PDFs or ZIP for multi-page/multiple PDFs). The web UI includes four tabs: CSV → Excel, PNG → WebP, WebP → PNG, and PDF → PNG.
